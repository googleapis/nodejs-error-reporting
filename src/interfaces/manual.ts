...
export function handlerSetup(client, config, logger) {
   ...
   * application code.
   * @param {Any|ErrorMessage1} err - error information of any type or content.
   *  This can be of any type but by giving an instance of ErrorMessage1 as the
   ...
   * @returns {ErrorMessage1} - returns the error message created through with
  ...
  function reportManualError(err, request, additionalMessage, callback) {
    ...
    if (err instanceof ErrorMessage1) {
      ...
      if (err._autoGeneratedStackTrace) {
        ...
        // Delete the property so that if the ErrorMessage1 is reported a second
        // time, the stack trace is not appended a second time.  Also, the API
        // will not accept the ErrorMessage1 if it has additional properties.
